...
def year_sort_key(year_str: str) -> int:
    """Extract the first 4-digit year from a string like '2019/2020' for sorting."""
    match = re.search(r'\d{4}', str(year_str))
    return int(match.group()) if match else 9999


def build_position_map(all_records: list[dict]) -> dict[str, list[dict]]:
    """Group records by each individual position, sorted chronologically."""
    ...
    for pos in pos_map:
        pos_map[pos].sort(key=lambda r: year_sort_key(r["year"]))
    return pos_map
